...
def gaussian_blur_fft(x: torch.Tensor, sigma: float) -> torch.Tensor:
    ...
    Accepts (B, C, H, W) or (C, H, W) tensors of any range.
    """
    if sigma <= 0:
        return x.clone()
...
def gaussian_blur_adjoint(x: torch.Tensor, sigma: float) -> torch.Tensor:
